...
alldoc = database.flights.aggregate([{'$group' : {'_id':"$AIRLINE", 'DIVERSIONS':{'$sum':"$DIVERTED"}}},{'$sort':{"DIVERSIONS":-1}}])
for item in alldoc:
  print(item)
...
for score in list(database.flights.aggregate( 
            [
               ...
               {'$group' : {'_id':"$AIRLINE", 'DIVERSIONS':{'$sum':"$DIVERTED"}}},{'$sort':{"DIVERSIONS":-1}}
            ]
        )):
    scores.append(score)
